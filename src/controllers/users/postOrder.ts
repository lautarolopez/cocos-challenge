import type { PostOrderPayload } from '../../schemas/users/index.js';
...
export const postOrder = async ({
  ...
  if (finalOrderSize < 1) {
    throw new HttpError(
      'El monto total es insuficiente para adquirir al menos 1 acción',
      400,
    );
  }
...
const getEffectivePrice = async (
  ...
  if (type === OrderType.LIMIT && price) return price;
...
const getOrderCreationStatus = async ({
  instrumentId,
  side,
  userId,
  effectivePrice,
  ...
  switch (side) {
    ...
    case OrderSide.CASH_OUT: {
      const sufficientFunds = await checkAgainstCash(
        userId,
        effectivePrice * finalOrderSize,
      );
      if (!sufficientFunds) {
    ...
    case OrderSide.SELL: {
      const sufficientShares = await checkAgainstShares(
        userId,
        instrumentId,
        finalOrderSize,
      );
...
const checkAgainstCash = async (userId: number, cost: number) => {
...
const checkAgainstShares = async (
  userId: number,
  instrumentId: number,
  size: number,
) => {
